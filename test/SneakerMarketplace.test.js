...
describe("SneakerToken & SneakerMarketplace Integration", function () {
  ...
  describe("▶️ Physical Redemption", function() {
    it("Full owner can redeem physical sneaker", async function() {
      ...
      await marketplace.connect(seller).listSneaker(tokenId, 1, ethers.parseEther("1"), false);
     
      // Buy whole sneaker
      await marketplace.connect(buyer1).purchaseSneaker(tokenId, 1, {value: ethers.parseEther("1")});
    ...
    it("Cannot redeem fractional shares", async function() {
      ...
      await marketplace.connect(seller).listSneaker(tokenId, 100, ethers.parseEther("0.1"), true);
      await marketplace.connect(buyer1).purchaseSneaker(tokenId, 50, {value: ethers.parseEther("5")});
  ...
  describe("▶️ 100 Share Standardization", function() {
    it("Follows 1 token or 100 shares model", async function() {
      ...
      const fractionalTokenId = await mintAndGetId(seller, 100, "ipfs://fractional");
