...
describe("SneakerToken & SneakerMarketplace Integration", function () {
  ...
  describe("▶️ Minting", function () {
    it("Owner can mint and `sneakerOwners` is updated", async function () {
      ...
      const owners = await token.sneakerOwners(tokenId);
      expect(owners).to.include(seller.address);
    });
  ...
  describe("▶️ Listing & Unlisting", function () {
    ...
    it("Seller lists whole NFT (non‑fractional) correctly", async function () {
      ...
      expect(await token.balanceOf(marketplace.address, this.tokenId)).to.equal(
        50
      );
  ...
  describe("▶️ Direct Purchase & Commission", function () {
    ...
    it("Buyer can purchase and ETH is escrowed", async function () {
      const listing = await marketplace.getListing(this.tokenId);
      const total = listing.price * listing.shareAmt; // 2 ETH * 30
      const fee = (total * COMMISSION) / 100; // 5% cut
    ...
    it("Seller & owner can withdraw their ETH", async function () {
      const listing = await marketplace.getListing(this.tokenId);
      const total = listing.price * listing.shareAmt;
      const fee = (total * COMMISSION) / 100;

      // buyer purchases only 10 shares
      await marketplace
        .connect(buyer1)
        .purchaseSneaker(this.tokenId, 10, { value: listing.price * 10 });
      ...
      const gas1 = r1.gasUsed * r1.effectiveGasPrice;
      const afterSeller = await ethers.provider.getBalance(seller.address);

      expect(afterSeller + gas1 - beforeSeller).to.equal(
        (listing.price * 10 * (100 - COMMISSION)) / 100n
      );
      ...
      const gas2 = r2.gasUsed * r2.effectiveGasPrice;
      const afterOwner = await ethers.provider.getBalance(owner.address);

      expect(afterOwner + gas2 - beforeOwner).to.equal(
        (listing.price * 10n * COMMISSION) / 100n
      );
    });
    ...
    it("Reverts if insufficient ETH", async function () {
      ...
      await expect(
        marketplace
          .connect(buyer2)
          .purchaseSneaker(this.tokenId, 5, { value: listing.price * 4 })
      ).to.be.revertedWith("Insufficient funds to purchase this sneaker");
    });
  });

  describe("▶️ Auction / Bidding Flow", function () {
    beforeEach(async function () {
      this.tokenId = await mintAndGetId(seller, 1, "ipfs://auction");
      // Seller lists the *whole* NFT (only non-fractional auctions)
      await marketplace
        .connect(seller)
        .listSneaker(this.tokenId, 1, ethers.parseEther("3.0"), false);
    });

    it("Can place, accept, finalize an auction", async function () {
      // 1) Buyer1 places initial bid of 2 ETH
      await expect(
      ...
      // 2) Seller accepts that bid → starts 7-day window
      await expect(marketplace.connect(seller).acceptBid(this.tokenId, 1))
        ...
        .withArgs(
          ...
          // we just check that expiry is > now
          () => true
        );

      // 3) Buyer2 outbids with 2.5 ETH
      await expect(
      ...
      // 5) Finalize auction
      const listing = await marketplace.getListing(this.tokenId);
      const winningAmount = ethers.parseEther("2.5") * 1n;
      const fee = (winningAmount * COMMISSION) / 100n;
      const sellerShare = winningAmount - fee;

      await expect(marketplace.finaliseBid(this.tokenId))
        .to.emit(marketplace, "BidSucceeded")
        .withArgs(this.tokenId, buyer2.address, ethers.parseEther("2.5"));

      // NFT delivered to Buyer2
      expect(await token.balanceOf(buyer2.address, this.tokenId)).to.equal(1);

      // Seller and owner get escrowed ETH
      expect(await marketplace.pendingWithdrawals(seller.address)).to.equal(
        sellerShare
      );
      expect(await marketplace.pendingWithdrawals(owner.address)).to.equal(fee);
